                ...
                <div class="btn-container flex justify-between">
                    ...
                    <div class="botones">
                        ...
                        <a class="rounded bg-gray-500 text-white hover:bg-gray-400 w-auto p-2 mb-2" type="button"
                            href="{{ route('alumnos.create') }}">Agregar nuevo usuario</a>
                    </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($alumnos as $alumno)
                                <tr class="border-b hover:bg-slate-100">
                                    <td class="p-3 px-5">{{ $alumno->id }}</td>
                                    <td class="p-3 px-5">{{ $alumno->dui }}</td>
                                    <td class="p-3 px-5">{{ $alumno->nombre }}</td>
                                    <td class="p-3 px-5">{{ $alumno->correo }}</td>
                                    <td class="p-3 px-5">{{ $alumno->direccion }}</td>
                                    <td class="p-3 px-5">{{ $alumno->fecha }}</td>
                                    ...
                                    <td class="p-3 px-5 flex justify-start">
                                        <a type="button" href="{{ route('alumnos.edit', $alumno->id) }}"
                                            ...
                                            <form id="deleteForm{{ $alumno->id }}"
                                                action="{{ route('alumnos.destroy', $alumno->id) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
